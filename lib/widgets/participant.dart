...
class LocalParticipantWidget extends ParticipantWidget {
  ...
  const LocalParticipantWidget(this.participant, this.type, this.showStatsLayer, {super.key});
...
class RemoteParticipantWidget extends ParticipantWidget {
  ...
  const RemoteParticipantWidget(this.participant, this.type, this.showStatsLayer, {super.key});
...
abstract class _ParticipantWidgetState<T extends ParticipantWidget> extends State<T> {
  ...
  Widget build(BuildContext ctx) => Container(
    foregroundDecoration: BoxDecoration(
      border: widget.participant.isSpeaking && !isScreenShare ? Border.all(width: 5, color: LKColors.lkBlue) : null,
    ),
      ...
      children: [
        ...
        InkWell(
          ...
          child: activeVideoTrack != null && !activeVideoTrack!.muted
              ? VideoTrackRenderer(renderMode: VideoRenderMode.auto, activeVideoTrack!)
              : const NoVideoWidget(),
        ),
        // Bottom bar
        Align(
  ...
  List<Widget> extraWidgets(bool isScreenShare) => [
    Row(
      ...
      children: [
        ...
        if (videoPublication != null)
          RemoteTrackQualityMenuWidget(pub: videoPublication!, icon: Icons.monitor_outlined),
      ],
    ),
  ];
}
    ...
    child: PopupMenuButton<Function>(
      ...
      itemBuilder: (BuildContext context) => <PopupMenuEntry<Function>>[
        ...
        if (pub.subscribed == false)
          PopupMenuItem(child: const Text('Subscribe'), value: () => pub.subscribe())
        else if (pub.subscribed == true)
          PopupMenuItem(child: const Text('Un-subscribe'), value: () => pub.unsubscribe()),
      ],
    ),
  );
}
